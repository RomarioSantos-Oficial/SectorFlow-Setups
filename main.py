...
class VirtualEngineer:
    ...
    def request_suggestion(self) -> tuple[dict[str, int], list[str]]:
        ...
        # Se a IA não gerou deltas ou confiança baixa, usa heurísticas
        if not ai_deltas or confidence < 0.7:
            tele_dict = self._features_to_dict(features)
            heuristic_list = analyze_telemetry_advanced(
                telemetry_data=tele_dict,
                car_class=self._car_class,
                current_setup=self._current_svm.params if self._current_svm else {},
                session_type=self.telemetry.get_session_type(),
    ...
    def generate_race_from_quali(
        ...
        for mf in model_files:
            try:
                ...
                logger.debug("Erro ao carregar modelo individual: %s", e)
    ...
    def shutdown(self):
        ...
        try:
            if self._car_name and self._track_name:
                car_id = self.db.get_or_create_car(self._car_name, self._car_class)
                track_id = self.db.get_or_create_track(self._track_name)
                self.normalizer.save(car_id, track_id, base_dir=MODELS_DIR)
        except Exception as e:
