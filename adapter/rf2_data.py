...
class Vehicle(DataAdapter):
    ...
    def vehicle_name(self, index: int | None = None) -> str:
        """Vehicle name from scoring (usually team/entry name in LMU)"""
        return tostr(self.shmm.rf2ScorVeh(index).mVehicleName)

    def vehicle_model(self, index: int | None = None) -> str:
        """Vehicle model name from telemetry (actual car model in LMU)"""
        try:
            return tostr(self.shmm.rf2TeleVeh(index).mVehicleName)
