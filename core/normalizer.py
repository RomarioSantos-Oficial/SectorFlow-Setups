...
class FeatureNormalizer:
    ...
    def save(self, car_id: str, track_id: str, base_dir: str = "models"):
        """Salva os parâmetros de normalização."""
        save_path = Path(base_dir) / car_id / f"normalizer_{track_id}.npz"
    ...
    def load(self, car_id: str, track_id: str, base_dir: str = "models"):
        """Carrega parâmetros de normalização salvos."""
        load_path = Path(base_dir) / car_id / f"normalizer_{track_id}.npz"
        if not load_path.exists():
            logger.info("Normalizer não encontrado em %s", load_path)
            return
