...
ADJUSTMENT_HIERARCHY = {
    "aero": {
        ...
        "params": [
            ...
            "delta_ride_height_f",
            "delta_ride_height_r",
            "delta_ride_height_fl",
            "delta_ride_height_fr",
            "delta_ride_height_rl",
            "delta_ride_height_rr",
            "delta_brake_duct_f",
            "delta_brake_duct_r",
            "delta_radiator",
        ],
    ...
    "dampers": {
        ...
        "params": [
            "delta_slow_bump_f",
            "delta_slow_bump_r",
            "delta_slow_rebound_f",
            "delta_slow_rebound_r",
            "delta_fast_bump_f",
            "delta_fast_bump_r",
            "delta_fast_rebound_f",
            "delta_fast_rebound_r",
        ],
    },
    "electronics": {
        "order": 4,
...
def apply_rules(car_class: str) -> tuple[str, list[str]]:
    ...
    priority = _STRATEGY_PARAM_PRIORITY.get(
        strategy, _STRATEGY_PARAM_PRIORITY["MECHANICAL_GRIP"]
    )
    return strategy, list(priority)


def sort_suggestions_by_class(
    suggestions: list["HeuristicSuggestion"], car_class: str
    ...
    return sorted(
        suggestions,
        key=lambda s: (cat_index.get(s.category, len(priority_order)), s.priority),
    )
...
def analyze_telemetry_advanced(
    ...
    track_name: str,
    driver_profile: dict,
    db_manager: DatabaseManager | None = None,  # Tipagem adicionada
) -> list[HeuristicSuggestion]:
    suggestions: list[HeuristicSuggestion] = []

    class_cfg = CLASS_CONFIG.get(car_class.lower(), {})

    # Calibração por Pista para Heurísticas
    ...
    # CORREÇÃO: Usar variáveis locais para os targets, sem mutar CLASS_CONFIG
    base_temp_target = class_cfg.get("temp_target_c", (70, 90))
    target_temp_min = base_temp_target[0] + tire_temp_adjust
    target_temp_max = base_temp_target[1] + tire_temp_adjust

    base_brake_max = class_cfg.get("brake_temp_max_c", 700)
    target_brake_max = base_brake_max + brake_temp_adjust

    # ---------------------------------------------------------------------------
    # G1.2 — Pneus Michelin: Pressão e Temperatura Alvo (Loop Dinâmico)
    # ---------------------------------------------------------------------------
    ...
    # Iterar pelos 4 pneus dinamicamente
    wheels = [
        ("fl", "dianteiro esquerdo", "f"),
        ("fr", "dianteiro direito", "f"),
        ("rl", "traseiro esquerdo", "r"),
        ("rr", "traseiro direito", "r"),
    ]

    for wheel, side_name, axis in wheels:
        pressure = telemetry_data.get(f"{wheel}_pressure_avg", 0.0)
        temp_inner = telemetry_data.get(f"{wheel}_temp_inner_avg", 0.0)
        temp_middle = telemetry_data.get(f"{wheel}_temp_middle_avg", 0.0)
        temp_outer = telemetry_data.get(f"{wheel}_temp_outer_avg", 0.0)

        # Ignorar se os dados vierem zerados (ex: outlap)
        if pressure <= 0 or temp_inner <= 0:
            continue

        # Regras de Pressão
        if pressure > tire_pressure_target_kpa[1] + 7:  # Tolerância ajustada para 7kPa
            suggestions.append(
                HeuristicSuggestion(
                    param_name=f"delta_pressure_{wheel}",
                    delta=-1,
                    rule_name="TIRE_PRESSURE_TOO_HIGH",
                    condition=f"Pressão {wheel.upper()} {pressure:.0f} kPa (alvo máx {tire_pressure_target_kpa[1]} kPa)",
                    explanation=f"Reduzir pressão do pneu {side_name} para atingir a janela ideal.",
                    priority=1,
                    category="fine_tune",
                )
            )
        elif pressure < tire_pressure_target_kpa[0] - 7:
            suggestions.append(
                HeuristicSuggestion(
                    param_name=f"delta_pressure_{wheel}",
                    delta=+1,
                    rule_name="TIRE_PRESSURE_TOO_LOW",
                    condition=f"Pressão {wheel.upper()} {pressure:.0f} kPa (alvo min {tire_pressure_target_kpa[0]} kPa)",
                    explanation=f"Aumentar pressão do pneu {side_name} para atingir a janela ideal.",
                    priority=1,
                    category="fine_tune",
                )
            )

        # Regras de Temperatura e Dutos de Freio
        temp_avg = (temp_inner + temp_middle + temp_outer) / 3.0
        if temp_avg > target_temp_max + 5:
            suggestions.append(
                HeuristicSuggestion(
                    param_name=f"delta_brake_duct_{axis}",
                    delta=+1,
                    rule_name="TIRE_TEMP_TOO_HIGH",
                    condition=f"Temp {wheel.upper()} {temp_avg:.0f}°C (alvo máx {target_temp_max}°C)",
                    explanation=f"Aumentar duto de freio {side_name.split()[0]} para ajudar a resfriar o pneu.",
                    priority=2,
                    category="aero",
                )
            )
        elif temp_avg < target_temp_min - 5:
            suggestions.append(
                HeuristicSuggestion(
                    param_name=f"delta_brake_duct_{axis}",
                    delta=-1,
                    rule_name="TIRE_TEMP_TOO_LOW",
                    condition=f"Temp {wheel.upper()} {temp_avg:.0f}°C (alvo min {target_temp_min}°C)",
                    explanation=f"Reduzir duto de freio {side_name.split()[0]} para aquecer o pneu.",
                    priority=2,
                    category="aero",
                )
            )

        # Regras de Spread (Camber)
        temp_spread = max(temp_inner, temp_middle, temp_outer) - min(
            temp_inner, temp_middle, temp_outer
        )
        if temp_spread > temp_spread_max_c + 2:
            if temp_outer > temp_inner:
                suggestions.append(
                    HeuristicSuggestion(
                        param_name=f"delta_camber_{wheel}",
                        delta=+1,
                        rule_name="TIRE_SPREAD_OUTER_HOT",
                        condition=f"Spread {wheel.upper()} {temp_spread:.0f}°C (outer > inner)",
                        explanation=f"Reduzir camber negativo do pneu {side_name} para distribuir melhor a temperatura.",
                        priority=1,
                        category="fine_tune",
                    )
                )
            elif temp_inner > temp_outer:
                suggestions.append(
                    HeuristicSuggestion(
                        param_name=f"delta_camber_{wheel}",
                        delta=-1,
                        rule_name="TIRE_SPREAD_INNER_HOT",
                        condition=f"Spread {wheel.upper()} {temp_spread:.0f}°C (inner > outer)",
                        explanation=f"Aumentar camber negativo do pneu {side_name} para distribuir melhor a temperatura.",
                        priority=1,
                        category="fine_tune",
                    )
                )
    ...
    # ---------------------------------------------------------------------------
    if (
        car_class.lower() in ["hypercar", "lmp2"]
        and class_cfg.get("ride_height_sensitivity") == "high"
    ):
        ...
        if (
            telemetry_data.get("oversteer_high_speed", False) and rake > 0.02
        ):  # Exemplo: rake > 2cm
            suggestions.append(
                HeuristicSuggestion(
                    param_name="delta_ride_height_r",
                    delta=-1,
                    rule_name="AERO_RAKE_HIGH_SPEED_INSTABILITY",
        ...
        # E o rake for muito baixo (frente muito alta em relação à traseira)
            ...
            telemetry_data.get("understeer_high_speed", False) and rake < -0.01
        ):  # Exemplo: rake < -1cm
            suggestions.append(
                HeuristicSuggestion(
                    param_name="delta_ride_height_r",
                    delta=+1,
                    rule_name="AERO_RAKE_HIGH_SPEED_UNDERSTEER",
    ...
    if "lmgt3" in car_class.lower() or "gte" in car_class.lower():
        ...
        if (
            "bmw" in car_class.lower() or "aston" in car_class.lower()
        ):  # Exemplo de detecção de marca
            if telemetry_data.get("understeer_entry", False) or telemetry_data.get(
                "understeer_mid", False
            ):
                suggestions.append(
                    HeuristicSuggestion(
                        param_name="delta_spring_f",
                        delta=-1,
                        rule_name="GT_FRONT_ENGINE_UNDERSTEER",
        ...
        if (
            "ferrari" in car_class.lower() or "porsche" in car_class.lower()
        ):  # Exemplo de detecção de marca
            if telemetry_data.get("oversteer_exit", False) or telemetry_data.get(
                "oversteer_mid", False
            ):
                suggestions.append(
                    HeuristicSuggestion(
                        ...
                        delta=+1,
                        rule_name="GT_MID_ENGINE_OVERSTEER",
    ...
    if "le mans" in track_name.lower():
        # Priorizar low drag
        if (
            telemetry_data.get("max_speed_on_straight", 0.0) < 300
        ):  # Exemplo: se a velocidade de reta for baixa
            suggestions.append(
                HeuristicSuggestion(
                    param_name="delta_rw",
                    delta=-1,
                    rule_name="TRACK_LE_MANS_LOW_DRAG",
    ...
    if car_class.lower() in ["lmgt3", "gte"] and class_cfg.get("has_abs"):
        abs_intervention_freq = telemetry_data.get(
            "abs_intervention_frequency", 0.0
        )  # 0-1
        brake_lock_freq = telemetry_data.get("brake_lock_frequency", 0.0)  # 0-1

        if (
            abs_intervention_freq > 0.7 or brake_lock_freq > 0.3
        ):  # ABS intervindo muito ou travando muito
            suggestions.append(
                HeuristicSuggestion(
                    param_name="delta_brake_bias",
                    delta=+1,
                    rule_name="ABS_TC_BRAKE_LOCK",
            ...
            suggestions.append(
                HeuristicSuggestion(
                    param_name="delta_brake_pressure",
                    delta=-1,
                    rule_name="ABS_TC_BRAKE_LOCK",
                    condition=f"ABS intervindo muito ({abs_intervention_freq:.1%}) ou travando ({brake_lock_freq:.1%})",
    ...
    if car_class.lower() in ["lmgt3", "gte"] and class_cfg.get("has_tc"):
        tc_intervention_freq = telemetry_data.get(
            "tc_intervention_frequency", 0.0
        )  # 0-1
        wheelspin_freq = telemetry_data.get("wheelspin_frequency", 0.0)  # 0-1

        if (
            tc_intervention_freq > 0.7 or wheelspin_freq > 0.3
        ):  # TC intervindo muito ou muito wheelspin
            suggestions.append(
                HeuristicSuggestion(
                    param_name="delta_tc_map",
                    delta=+1,
                    rule_name="ABS_TC_WHEELSPIN",
    ...
    if avg_tire_temp < temp_target_c[0] - 10:  # 10C abaixo do limite inferior
        suggestions.append(
            HeuristicSuggestion(
                param_name="delta_pressure_fl",
                delta=+1,
                rule_name="TIRE_WARMUP_COLD_TYRES",
...
def _check_thermal_trend(
    ...
    slopes = [
        fl_outer_temps[i] - fl_outer_temps[i - 1] for i in range(1, len(fl_outer_temps))
    ]
...
def _check_camber(tele: dict, config: dict, suggestions: list[HeuristicSuggestion]):
...
def _check_tire_pressure(
    tele: dict, config: dict, suggestions: list[HeuristicSuggestion]
):
...
def _check_rear_wing(tele: dict, config: dict, suggestions: list[HeuristicSuggestion]):
...
def _check_springs(tele: dict, config: dict, suggestions: list[HeuristicSuggestion]):
...
def _check_brakes(tele: dict, config: dict, suggestions: list[HeuristicSuggestion]):
...
def _check_weather(tele: dict, config: dict, suggestions: list[HeuristicSuggestion]):
...
def _check_compound(tele: dict, config: dict, suggestions: list[HeuristicSuggestion]):
...
def _check_rake(tele: dict, config: dict, suggestions: list[HeuristicSuggestion]):
    """
...
def _check_fast_bump(tele: dict, config: dict, suggestions: list[HeuristicSuggestion]):
    """
...
def _check_brake_duct(tele: dict, config: dict, suggestions: list[HeuristicSuggestion]):
    """
...
def _check_abs_tc(tele: dict, config: dict, suggestions: list[HeuristicSuggestion]):
    """
        ...
        if rain > 0.3 or grip_avg < 0.7:
            suggestions.append(
                HeuristicSuggestion(
                    ...
                    condition=(
                        f"rain({rain:.2f}) > 0.3 ou " f"grip({grip_avg:.2f}) < 0.7"
                    ),
                    explanation=(
...
def _check_corner_phase_feedback(
    tele: dict, config: dict, suggestions: list[HeuristicSuggestion]
):
    """
...
def merge_suggestions(
    heuristic: list[HeuristicSuggestion],
    ...
    # 1. Consolidar heurísticas (soma os deltas de regras diferentes para o mesmo parâmetro)
    for s in heuristic:
        current = final.get(s.param_name, 0)
        # Limita o delta consolidado a +/- 3 para evitar ajustes extremos de uma vez
        new_delta = max(min(current + s.delta, 3), -3)
        final[s.param_name] = new_delta

    # 2. IA Override (Se a confiança for alta, a IA esmaga a heurística determinística)
    if ai_deltas and ai_confidence > 0.7:
        for param, delta in ai_deltas.items():
            final[param] = delta

    # 3. Aplicar modificadores de sessão (Qualy vs Race)
    modifiers = SESSION_MODIFIERS.get(session_type, {})
    for param, mod_delta in modifiers.items():
        if param in final:
            final[param] += mod_delta
        else:
            final[param] = mod_delta

    # 4. Limitar por hierarquia/max_categories (Opcional: manter o piloto focado em poucos ajustes)
    # Agrupar as sugestões ativas por categoria
    active_categories = {}
    for param in final.keys():
        cat = get_param_category(param)
        if cat not in active_categories:
            active_categories[cat] = []
        active_categories[cat].append(param)

    # Se ultrapassar max_categories, remove as categorias menos prioritárias
    if len(active_categories) > max_categories:
        sorted_cats = sorted(active_categories.keys(), key=get_category_order)
        allowed_cats = set(sorted_cats[:max_categories])

        # Filtra o dicionário final para manter apenas parâmetros das categorias permitidas
        final = {
            p: d for p, d in final.items() if get_param_category(p) in allowed_cats
        }

    return final


def apply_driver_profile(
    deltas: dict[str, int],
...
def boost_from_learning_rules(
    deltas: dict[str, int], effective_rules: list[dict], failed_rules: list[dict]
) -> dict[str, int]:
    """
